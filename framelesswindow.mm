...
CFramelessWindow::CFramelessWindow(QWidget *parent)
    : QMainWindow(parent), m_draggableHeight(0), m_bWinMoving(false),
      m_bMousePressed(false), m_bCloseBtnQuit(true), m_bNativeSystemBtn(false),
      m_bIsCloseBtnEnabled(true), m_bIsMinBtnEnabled(true),
      m_bIsZoomBtnEnabled(true), m_bTitleBarVisible(false) {
  initUI();
}

//此类用于支持重载系统按钮的行为
// this Objective-c class is used to override the action of sysytem close button
// and zoom button
...
void CFramelessWindow::initUI() {
  ...
  //设置view扩展到标题栏
  window.styleMask |=
      NSWindowStyleMaskFullSizeContentView; // MAC_10_10及以上版本支持
...
void CFramelessWindow::onRestoreFromFullScreen() { setTitlebarVisible(false); }
